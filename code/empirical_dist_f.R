...
plot_scores <- function(max_scores, str, title="", savePlot=FALSE) {
  ...
  if (savePlot) {
    ggsave(paste0("p1_",str,".png"), p1)
    ggsave(paste0("p2_",str,".png"), p2)
  }
  p1
}
...
plot_entropy_hist <- function(entropy_df, str, title="",savePlot=TRUE) {
  ...
  entropy_hist
  if (savePlot) {
    ggsave(paste0("entropy_hist_",str,".png"), entropy_hist)
  }
  entropy_hist
}
...
plot_f1_hist <- function(S) {
  S %>% ggplot() +
    geom_histogram(aes(x=f1),bins=100, fill="black") +
    geom_vline(aes(xintercept=mean(f1)), color="firebrick", size=1)
}
plot_f2_hist <- function(S) {
  S %>% ggplot() +
    geom_histogram(aes(x=f2), bins=500, fill="black") +
    geom_vline(aes(xintercept=mean(f2)), color="firebrick", size=1)
