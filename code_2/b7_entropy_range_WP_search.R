...
df_entropy_conversion = read_csv("df_entropy_conversion.csv")

H_stars_espn = numeric(NUM_RUNS)
H_stars_num_correct = numeric(NUM_RUNS)
...
for (RUN in 1:NUM_RUNS) {
  ...
  ### find best (-inf, hU] or [hL, -inf)
  wp_hU_espn = numeric(nrow(df_entropy_conversion))
  wp_hU_num_correct = wp_hU_espn
  for (i in 1:nrow(df_entropy_conversion)) {
    print(paste0("run = ", RUN, " of ", NUM_RUNS, ", i = ", i, " of ", nrow(df_entropy_conversion)))
    
    if (df_entropy_conversion$left_tail[i]) {
      ### sample brackets from left tail of entropy distribution
      hL = -Inf
      hU = df_entropy_conversion$h[i]
    } else {
      ### sample brackets from right tail of entropy distribution
      hL = df_entropy_conversion$h[i]
      hU = Inf
    }
    our_submitted_backets = sample_n_brackets_entropyRange(PARAMS$n, hL, hU, prob_method="P_538_2022") 
  ...
  H_star_espn = df_entropy_conversion$H[first(which(wp_hU_espn == max(wp_hU_espn)))]
  H_star_num_correct = df_entropy_conversion$H[first(which(wp_hU_num_correct == max(wp_hU_num_correct)))]
  print(H_star_espn); print(H_star_num_correct);
  H_stars_espn[RUN] = H_star_espn
  H_stars_num_correct[RUN] = H_star_num_correct
}

######
H_star = tibble(H_stars_espn, H_stars_num_correct) %>%
  summarise(
    H_star_espn = mean(H_stars_espn),
    H_star_num_correct = mean(H_stars_num_correct)
  )
