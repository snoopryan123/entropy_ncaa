...
cdfMaxScore_q_chalky <- function(m,n,p,q,k) {
  # print("a2")
...
### check
# cdfMaxScore_q_chalky(m=m,n=100,p=0.7,q=0.5,k=30)
...
eMaxScore_q_chalky <- function(m,n,p,q) {
  ...
  for (k in 0:m) {
    ...
    vec[k+1] = 1 - cdfMaxScore_q_chalky(m,n,p,q,k)
  }
  sum(vec)
}
### check
...
ns = 10^(0:8)
xxxx = numeric(length(ns))
for (j in 1:length(ns)) {
  xxxx[j] = eMaxScore_q_chalky(m, n=ns[j], p=0.7, q=0.8)
}
xxxx
