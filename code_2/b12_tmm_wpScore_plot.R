...
for (p_ in unique(plot_df_tmmWpHam4$p)) {
  for (k_ in unique(plot_df_tmmWpHam4$k)) {
    ...
    # plot_wp_nkp
    ggsave(
      paste0(output_folder, "plot_wpTmmHammingScore_v4_k",format(k_, scientific=T),"_n",format(n_, scientific=T),"_p",p_,".png"), 
      plot_wp_nkp4,
      # width=14,height=10
      width=28,height=26
    )
...
plot_df_tmmWpHam5 = read_csv(paste0(output_folder,"plot_grid_wpScore_v5.csv"))
plot_df_tmmWpHam5 = plot_df_tmmWpHam5 %>%
  group_by(p,qE,qL,rE,rL,q_cutoff,r_cutoff,n,k,scoring_method) %>%
  mutate(wp = mean(wp)) %>%
  ungroup() %>%
  filter(fold == 1) 

for (p_ in unique(plot_df_tmmWpHam5$p)) {
  for (k_ in unique(plot_df_tmmWpHam5$k)) {
    n_ = k_
    ...
    # plot_wp_nkp
    ggsave(
      paste0(output_folder, "plot_wpTmmESPNScore_v5_k",format(k_, scientific=T),"_n",format(n_, scientific=T),"_p",p_,".png"), 
      plot_wp_nkp5,
      # width=12,height=10
      width=28,height=26
    )
